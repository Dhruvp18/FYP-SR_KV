...
def build_notebook(phase: int, *, repo: str, model: str, model3b: str, budget: float,
    ...
    cells = [
        ...
        code("import subprocess, sys",
             ...
             "LOGFILE = '/kaggle/working/run.log'",
             "",
             "def sh(cmd, cwd=WORKDIR, check=True):",
             "    \"\"\"Run a shell command, streaming output to the console AND to a",
             "    persistent /kaggle/working/run.log, so a traceback survives even if",
             "    the kernel ends in an error state. `kaggle kernels output` reliably",
             "    returns files written under /kaggle/working; it does NOT reliably",
             "    return a session log for an errored run (confirmed empirically -",
             "    see HANDOFF.md's 'tee everything to a file' lesson, which this",
             "    generated notebook did not yet follow before this fix).\"\"\"",
