...
scripts/
  kaggle_kernel.py        generate/push/poll/pull Kaggle kernels per phase
  checkpoint_utils.py     incremental save/resume + shard partitioning
  make_plots.py           every report figure, from results/
  check_results.py        the seven phase gates, completeness, ablation sanity
...
python -m pytest -q                    # 115 tests, CPU only, no downloads
...
**Running on Kaggle: see [HANDOFF.md](HANDOFF.md)** for project state, the
remaining pipeline with its pass conditions, and agent-ready automation
(`scripts/kaggle_kernel.py run --phase N`). [KAGGLE.md](KAGGLE.md) covers the
manual browser workflow.

Each remaining phase has a machine-checkable gate:

```bash
python scripts/check_results.py gate --phase 5 --model qwen2.5-1.5b
# exit 0 = proceed, 1 = failed/incomplete, 2 = needs a human look
```
...
| 4 — RoPE position ablation | needs GPU. `make phase4` then `make freeze-rope` |
| 5 — factorial matrix + full sweep | needs GPU. `make phase5` |
| 6 — hyperparameter sweep + 3B transfer | needs GPU. `make phase6-sweep phase6-3b` |
| 7 — figures | code complete; `make phase7` runs against real results |

Every phase from 1 on runs on Kaggle via
`python scripts/kaggle_kernel.py run --phase N --user <kaggle-username>`, which
pushes a detached kernel, polls it, pulls the output, and exits with that
phase's gate status.
