# Running SR-KV on Kaggle (manual / browser workflow)

You have no local GPU, so everything from Phase 1 onward runs on Kaggle. This
is the operational guide for driving it by hand in the browser.

**To automate it instead — push kernels from the terminal, poll them, pull
results and check each phase's gate — see [HANDOFF.md](HANDOFF.md) and
`scripts/kaggle_kernel.py`. That is the path to prefer if you have an agent
doing the work.**

Local machine still does useful work: the whole test suite (115 tests) runs on
...
```bash
python -m pytest -q                        # 115 CPU tests, ~2 min, no GPU needed
